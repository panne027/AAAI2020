...
public class DiscretizerMain {
	...
	public static void main(String[] args) throws Exception {
		...
		PatternDimension AbsErrorDim = new PatternDimension();
		AbsErrorDim.name = "AbsError";
		...
		passDim.name = "Pass";
		PatternDimension NoxTheoryppmDim = new PatternDimension();
		NoxTheoryppmDim.name = "NoxTheoryppm";
		...
		for(int i=0;i<inputDimensions.length;i++)
		{
			...
			if(dimLine.length>5) //explicit event intervals are given
			{
		...
		try {
			...
			if((inLine=bfRead.readLine())!=null)
			{
				...
				for(int i = 0;i<columnNames.length;i++)
				{
					...
					else if(columnNames[i].equalsIgnoreCase("NOxTheoryppm"))
					{
						NoxTheoryppmDim.indexInInputFile = i;
					}
					...
					else if(columnNames[i].equalsIgnoreCase("AbsError"))
					{
						AbsErrorDim.indexInInputFile = i;
					}
		...
		try {
			...
			while(((inLine=bfRead.readLine())!=null) && (!inLine.equals("")))
			{
				...
				if(r.RelativeError<0) r.RelativeError = 0;
				r.NOxTheoryppm = Double.parseDouble(values[NoxTheoryppmDim.indexInInputFile]);
				if(r.NOxTheoryppm<0) r.NOxTheoryppm = 0;
				r.AbsError = Double.parseDouble(values[AbsErrorDim.indexInInputFile]);
				if(r.AbsError<0) r.AbsError = 0;
				...
				for(int j=0;j<dimensionsInfo.length;j++)
				{
						...
						if(dimensionsInfo[j].name.equalsIgnoreCase("NOxTheoryppm") || dimensionsInfo[j].name.equalsIgnoreCase("SCRoutgps") || dimensionsInfo[j].name.equalsIgnoreCase("Bkpwr") || dimensionsInfo[j].name.equalsIgnoreCase("SCRinppm") || dimensionsInfo[j].name.equalsIgnoreCase("SCRingps"))
						{
							if(val<0) val = 0;
						}
		...
		try {
			...
			bfWrite.write("Iteration,Year,Month,Day,Latitude,Longitude,RelativeError,NOxTheoryppm,AbsError,pass");
			for(int i=0;i<dimensionsInfo.length;i++)
			{
			...
			for(int i=0;i<allReadings.size();i++)
			{
				...
				bfWrite.write(r.Iteration+","+r.year+","+r.month+","+r.day+","+r.Latitude+","+r.Longitude+","+r.RelativeError+","+r.NOxTheoryppm+","+r.AbsError+","+r.pass);
				for(int j=0;j<dimensionsInfo.length;j++)
