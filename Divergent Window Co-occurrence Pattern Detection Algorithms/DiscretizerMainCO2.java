...
public class DiscretizerMainCO2 {
	...
	public static void main(String[] args) throws Exception {
		...
		//CO2 dimensions
		PatternDimension co2gpsDim = new PatternDimension();
		co2gpsDim.name = "CO2gps";
		...
		// by user in dimensionsInfo[]) in addition to the output co2x  (co2gps) and engine power (Engpwr) dimensions.
		
		
		try {
			System.out.println("Started reading header file..");
			...
			if((inLine=bfRead.readLine())!=null)
			{
				...
				for(int i = 0;i<columnNames.length;i++)
				{
					...
					else if(columnNames[i].equalsIgnoreCase("CO2gps"))
					{
						co2gpsDim.indexInInputFile = i;
					}
		...
		//MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM
		ArrayList<EngineReadingCO2> allReadings = new ArrayList<EngineReadingCO2>();
		...
		try {
			...
			while(((inLine=bfRead.readLine())!=null) && (!inLine.equals("")))
			{
				...
				EngineReadingCO2 r = new EngineReadingCO2();
				...
				r.co2gps = Double.parseDouble(values[co2gpsDim.indexInInputFile]);
				if(r.co2gps<0) r.co2gps = 0;
				...
				allReadings.add(r);
				lineCtr++;
				System.out.println("Done with line #"+lineCtr);
			}
			bfRead.close();
		} catch (IOException e) {
			System.out.println("Error! Cannot open/close data file.");
		...
		try {
			System.out.println("Done with discretization.");
			...
			bfWrite.write("Iteration,Year,Month,Day,Latitude,Longitude,co2gps,Bkpwr,pass");
			for(int i=0;i<dimensionsInfo.length;i++)
			{
			...
			{
				EngineReadingCO2 r = allReadings.get(i);
				bfWrite.write(r.Iteration+","+r.year+","+r.month+","+r.day+","+r.Latitude+","+r.Longitude+","+r.co2gps+","+r.Bkpwr+","+r.pass);
				for(int j=0;j<dimensionsInfo.length;j++)
