...
public class HybridMinerTransients {
	...
	public static void main(String[] args) throws IOException {
		...
		*/
		...
		if(boundWithLeavesOnly==1)
			outputFileName = "HybridMiner_BoundWithLeafNodesOnly_"+Double.toString(anomalyStandardSummationThreshold)+"_"+Integer.toString(lag)+"_"+Double.toString(anomalyPercentageThreshold)+"_"+Double.toString(kFunctionThreshold)+"_"+Double.toString(patternSupportThreshold)+"_"+Integer.toString(windowLength)+"_"+Integer.toString(minLength)+"_"+Integer.toString(maxLength)+"_"+Integer.toString(countingMethod)+"_"+Integer.toString(boundWithLeavesOnly);
		else
			outputFileName = "HybridMiner_BoundWithLastBottomLevel_"+Double.toString(anomalyStandardSummationThreshold)+"_"+Integer.toString(lag)+"_"+Double.toString(anomalyPercentageThreshold)+"_"+Double.toString(kFunctionThreshold)+"_"+Double.toString(patternSupportThreshold)+"_"+Integer.toString(windowLength)+"_"+Integer.toString(minLength)+"_"+Integer.toString(maxLength)+"_"+Integer.toString(countingMethod)+"_"+Integer.toString(boundWithLeavesOnly);
		...
		try {
			...
			while((inLine=bfRead.readLine())!=null)
			{
				...
				allReadings[rowIndex].AbsError = Double.parseDouble(values[8]);
		...
		String outputFileNameForanomalouswindows =  "";
		outputFileNameForanomalouswindows="AnomalousWindows"+'_'+Integer.toString(allReadings.length)+'_'+Double.toString(anomalyStandardSummationThreshold)+'_'+Double.toString(anomalyPercentageThreshold)+'_'+Integer.toString(windowLength)+".txt";

		printanomalouswindows(outputFileNameForanomalouswindows, anomalousWindows,anomalyStandardSummationThreshold,anomalyPercentageThreshold,windowLength);
		...
		//ArrayList<OutputPattern> topKFunctionValuePatterns = mineCooccurrencePatterns(minLength, maxLength,totalAnomaliesCount,anomalousWindows, lag, allReadings,dimensionsInfo,patternSupportThreshold,kFunctionThreshold,bfWrite,countingMethod,isMinSupPruningON);
			String outputanomalousname= Double.toString(allReadings.length)+'_'+Double.toString(dimensionsInfo.length)+'_'+Double.toString(patternSupportThreshold)+'_'+Double.toString(totalAnomaliesCount)+'_'+Double.toString(kFunctionThreshold)+'_'+Double.toString(countingMethod);

			ArrayList<OutputPattern> topKFunctionValuePatterns = mineCooccurrencePatterns(minLength, maxLength,totalAnomaliesCount,anomalousWindows, lag, allReadings,dimensionsInfo,patternSupportThreshold,kFunctionThreshold,bfWrite,countingMethod,boundWithLeavesOnly, outputanomalousname);
		
		//Print top-k Ripley's K-function value patterns
		String outputFileNameForTopKRipleysPatterns =  "";
		if(boundWithLeavesOnly==1)
			outputFileNameForTopKRipleysPatterns = "HybridMiner_BoundWithLeafNodesOnly_"+Double.toString(anomalyStandardSummationThreshold)+"_"+Double.toString(anomalyPercentageThreshold)+"_"+Double.toString(kFunctionThreshold)+"_"+Double.toString(patternSupportThreshold)+"_"+Integer.toString(minLength)+"_"+Integer.toString(maxLength)+Integer.toString(countingMethod)+Integer.toString(boundWithLeavesOnly)+"_TopKpatternsForRipleysK";
		else
			outputFileNameForTopKRipleysPatterns = 	"HybridMiner_BoundWithLastBottomLevel_"+Double.toString(anomalyStandardSummationThreshold)+"_"+Double.toString(anomalyPercentageThreshold)+"_"+Double.toString(kFunctionThreshold)+"_"+Double.toString(patternSupportThreshold)+"_"+Integer.toString(minLength)+"_"+Integer.toString(maxLength)+Integer.toString(countingMethod)+Integer.toString(boundWithLeavesOnly)+"_TopKpatternsForRipleysK";
	...
	private static boolean enumerate_with_minsupp_pruning(HybridDimensionsGraph curDimGraph, HybridDimensionNode node,
			...
			int boundWithLeavesOnly, HashMap<String, Integer> subsetsJoinSetCountsMap, String outputanomalousname) {
		...
		if(!enumeratedPatternsMap.containsKey(pattern)) //pattern not yet enumerated
    	{
    		int [] counts;
			if(countingMethod==1)
				counts = countPatternUsingLinearScan(allReadings, anomalousWindows, includedDimensions, windowFrom, windowTo, from, to, lag, outputanomalousname, totalAnomaliesCount);
			else
	...
	private static void enumerate_with_UB_pruning(HybridDimensionsGraph curDimGraph,
		...
		double kFunctionThreshold, BufferedWriter bfWrite,int boundWithLeavesOnly,HashMap<String, Integer> subsetsJoinSetCountsMap, String outputanomalousname)
		{
		...
		if(node==curDimGraph.root)
		{
	    	...
	    	{
				//String outputanomalousname= Double.toString(patternSupportThreshold)+'_'+Double.toString(totalAnomaliesCount)+'_'+Double.toString(kFunctionThreshold);
			...
			LinkedHashMap<String, TemporalWindow> anomalousWindows, EngineReading[] allReadings, PatternDimension[] dimensionsInfo, HashMap<String, String> enumeratedPatternsMap, HybridDimensionsGraph curDimGraph, int countingMethod, AllStatesGraph stateGraphs, double minsup, int totalAnomaliesCount, ArrayList<OutputPattern> topKFunctionValuePatterns, double kFunctionThreshold, BufferedWriter bfWrite, String outputanomalousname)
	{
		...
		{
			//String outputanomalousname= Integer.toString(allReadings.length)+'_'+Double.toString(minsup)+'_'+Integer.toString(totalAnomaliesCount)+'_'+Double.toString(kFunctionThreshold);
	...
	private static int[] countPatternUsingLinearScan(EngineReading[] allReadings, LinkedHashMap<String, TemporalWindow> anomalousWindows, ArrayList<Integer> includedDimensions, int windowFrom, int windowTo, int from, int to, int lag, String outputanomalousname, int totalAnomaliesCount )
	{
		...
		FileWriter file_to_write;
		BufferedWriter bfWrite;
		file_to_write = new FileWriter("windowspatterns"+outputanomalousname+ ".txt",true);
		bfWrite =  new BufferedWriter(file_to_write);

		//ArrayList<TemporalWindow> Patternwindows=null;
		...
		for (int j=0;j<allReadings.length-(to-from);j++) //compare starting from every step in the time instant
        {
        	...
        	if(isFound)
        	{
        		...
        		for(int d = 0;d<=lag;d++)
        		{
	            	...
	            	if(anomalousWindows.containsKey(windowKey)) //corresponding window is anomalous 
	            	{
	            		...
	            		//Patternwindows.add(anomalousWindows.get(windowKey));
						//bfWrite.write(Long.toString(anomalousWindows.get(windowKey).startIteration)+"\n");
						//bfWrite.write(Integer.toString(j+d)+"\n");
	            	}
        		}
        		if(didPatternOccurWithAnomaly)
				{	counts[2]++;
					bfWrite.write(Integer.toString(j)+"\n");
				}
        	}
        }
			...
			if(KFunctionDenominator>0 && allReadings.length>0)
			{
				...
				bfWrite.write(Double.toString(kValue)+"\n");}

		bfWrite.close();
	}
		catch(IOException e)
	{
		System.out.println("Error printing anomalous windows output file!\n");
		e.printStackTrace();
		return counts;
	}
	 ...
	 static LinkedHashMap<String, TemporalWindow> findAllAnomalousWindowsWithStandardThreshold(EngineReading[] allReadings,double anomalyStandardThreshold,int windowLength)
	{
			...
			for(int i=0,j=i+windowLength-1;((i<=allReadings.length-windowLength) && (j<allReadings.length));i++,j++)
			{
					...
					if(allReadings[j].Iteration-allReadings[i].Iteration+1==windowLength)
					{
						...
						for(int k=i;k<=j;k++)
						{
							...
							AbsErrorSum+=allReadings[k].AbsError;
							//SCRoutgpsSum+=allReadings[k].SCRoutgps;
							//bkpwrSum+=allReadings[k].Bkpwr;
						}
					
						if((!isSCROFF) &&  (AbsErrorSum>0)) //check if anomalous
						{
							...
							*/
							...
							double AbsErrorAvg = AbsErrorSum/windowLength;
							//if(maxNOX<NOxInGPKWHr)
								//maxNOX = NOxInGPKWHr;
							if(AbsErrorAvg>anomalyStandardThreshold)
							{
	 ...
	 static LinkedHashMap<String, TemporalWindow> findAllAnomalousWindowsWithSummationAndPercentageThresholds(EngineReading[] allReadings,double anomalyStandardThreshold,double anomalyPercentageThreshold, int windowLength) throws IOException {
		LinkedHashMap<String, TemporalWindow> anomalousWindows = new LinkedHashMap<String, TemporalWindow>();
			...
			for (int i = 0, j = i + windowLength - 1; ((i <= allReadings.length - windowLength) && (j < allReadings.length)); i++, j++) {
				...
				if (allReadings[j].Iteration - allReadings[i].Iteration + 1 == windowLength) {
					...
					for (int k = i; k <= j; k++) {
						...
						//SCRoutgpsSum+=allReadings[k].SCRoutgps;
						//bkpwrSum+=allReadings[k].Bkpwr;
						if (allReadings[k].AbsError>anomalyPercentageThreshold)
							counter++;

						//RelErrorSum += allReadings[k].RelativeError;
					}
					if(counter>=windowLength)
						for (int k = i; k <= j; k++) AbsErrorSum += allReadings[k].AbsError;

					if ((!isSCROFF) && (AbsErrorSum > 0) && (allReadings[i].AbsError > 0) && (counter>=windowLength)) //check if anomalous and percentage of Nox increased
					{//(allReadings[j].RelativeError > allReadings[i].RelativeError)
						//double increasePercentage = ((allReadings[j].NOxTheoryppm - allReadings[i].NOxTheoryppm) / allReadings[i].NOxTheoryppm) * 100;

						double AbsErrorAvg = AbsErrorSum / windowLength;
						double increasePercentage = AbsErrorAvg;

						//double increasePercentage = ((max-min)/min)*100;
						//double NOxInGPKWHr = (SCRoutgpsSum*3600)/bkpwrSum;

						boolean isAnomalous = false;
						if (anomalyStandardThreshold >= 0 && anomalyPercentageThreshold >= 0 && (AbsErrorAvg > anomalyStandardThreshold) && (increasePercentage > anomalyPercentageThreshold))
							isAnomalous = true;
	...
	private static void printanomalouswindows(String outputFileNameForanomalouswindows, LinkedHashMap<String, TemporalWindow> anomalousWindows,double anomalyStandardThreshold,double anomalyPercentageThreshold, int windowLength)
	{
		// Print top k patterns
		FileWriter file_to_write;
		BufferedWriter bfWrite;
		try {
			file_to_write = new FileWriter(outputFileNameForanomalouswindows);
			bfWrite =  new BufferedWriter(file_to_write);
			...
			for(String key : anomalousWindows.keySet())
			{
				...
				int windowFrom = Integer.parseInt(indexes[0]);
				int windowTo = Integer.parseInt(indexes[1]);
				bfWrite.write(Integer.toString(windowFrom)+"\n");
